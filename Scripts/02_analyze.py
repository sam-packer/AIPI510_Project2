...
sr_ci = result.confidence_interval(confidence_level=0.95)
print(f"95% confidence interval for Spread Ratio: [{sr_ci.low:.3f}, {sr_ci.high:.3f}]")
...
wlp_result = ttest_rel(wlp_pairs["toaster"], wlp_pairs["oven"])
wlp_ci = wlp_result.confidence_interval(confidence_level=0.95)
print(f"95% confidence interval for Weight Loss %: [{wlp_ci.low:.3f}, {wlp_ci.high:.3f}]")